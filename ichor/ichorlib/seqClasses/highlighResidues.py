...
def plotHighlightedSeq(ax, seq, aatohoghlight = 'K', blockfrom = 0, blockto = 0, **kwargs):
    ...
    for i, aa in enumerate(seq):
        if aa == aatohoghlight:
            # print i
            ln = ax.bar(i, height, width, color=cp.tableau20[6], edgecolor="none")
        else:
            ln = ax.bar(i, height, width, color=cp.tableau20[11], edgecolor="none")

    # color membrane topology
    if blockfrom > 0 and blockto > 0:
        for i, aa in enumerate(seq):
            if i >= blockfrom and i <= blockto:
                ln = ax.bar(i, height, width, color=cp.tableau20[2], alpha=0.5, edgecolor="none")

    return ln


fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1, figsize=(16, 4), sharex=True, sharey=True)
